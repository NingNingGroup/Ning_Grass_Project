...
async def main():
    ...
    email = input("[?] 您的Grass账号登陆邮箱 : ")
    password = input("[?] 您的Grass登陆密码 : ")
    ...
    if token is None:
        ...
        return
    open("userIdInfo.txt", "w").write(userid)
    open("loginTokenInfo.txt", "w").write(token)
    print(f"[+] 你现在已经登陆了Grass项目，现在你需要运行main.py脚本")
